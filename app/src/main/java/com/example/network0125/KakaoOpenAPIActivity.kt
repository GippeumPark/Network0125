    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        object : Thread() {
            override fun run() {
                ...
                //파라미터는 한글인 경우 인코딩을 해줘야 함
                val addr = "https://dapi.kakao.com/v3/search/book?query=" +
                        URLEncoder.encode("코틀린", "utf-8") + "&size=50"
                //다운로드 받을 URL 생성
                val url = URL(addr)
                ...
                con.setRequestProperty("Authorization", "KakaoAK 7bc7f9786acc063f77fe239bfe57477a")
                ...
                if(TextUtils.isEmpty(sb.toString())){
                    Toast.makeText(this@KakaoOpenAPIActivity, "쿼리의 횟수가 초과되거나 네트워크에 이상이 있습니다",
                        Toast.LENGTH_LONG).show()
                    return
                }else{
                    //문자열을 JSONObject 로 변환
                    //스마트폰은 JSON Parsing 못하면 아무것도 못씀
