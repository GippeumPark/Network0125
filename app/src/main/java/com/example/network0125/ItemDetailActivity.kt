...
/*
                ...
                //이미지를 다운로드 받기 위한 스레드를 생성해서 시작
                object:Thread(){
                    override fun run(){
                        //이미지 파일의 이름(map의 pictureurl)을 가져오기
                        val pictureurl = map!!["picture"] as String?
                        //이미지 파일의 URL을 생성
                        ...
                        //handler.sendMessage(msg) handler를 부를 수 없음, this는 자기자신이 되기 때문에 부를 수 없음
            ...
            override fun run(){
                ...
                while(true) {
                    ...
                    if(TextUtils.isEmpty(json)){
                        Toast.makeText(this@ItemDetailActivity, "데이터를 받아오지 못했습니다. 네트워크를 확인하세요",
                        Toast.LENGTH_LONG).show()
                        return
                    }else{
                        //문자열을 JSONObject로 변환
                        val jsonObject = JSONObject(json)
                        //결과에서 result의 값을 가져오면 데이터가 전송되었는지 확인 가능
                        //true이면 item 키에 데이터가 있는 것이고 false이면 없는 것
                        val result = jsonObject.getBoolean("result")
                        if(result==false){
                            Toast.makeText(this@ItemDetailActivity, "itemid나 파라미터가 잘못되었습니다.",
                            Toast.LENGTH_LONG).show()
                            return
                        }else{
                            //item 키에 해당하는 데이터를 JSONObject로 가져옴
